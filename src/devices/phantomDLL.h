...
//! the function returns 1 if everything went ok, a negative value otherwise (check list of errors)
//! Note that positions are expressed in mm with respect to a chai reference frame (x towards user, 
...
//! the function returns 1 if everything went ok, a negative value otherwise (check list of errors)
//! Note that forces are expressed in Newtons with respect to a chai reference frame (x towards user, 
//! y to the right, z vertical pointing up). Note: no safety features are implemented other than the 
...
//! ReadOrientMat2DOF(int num, double *m);, reads the orientation matrix of the stylus for a 2dof wristed
//! phantom device and returns it in a chai coordinate frame (x towards user, y to the right, z vertical pointing up)
FUNCTION int __stdcall   ReadOrientMat2DOF(int num, 
 									  double *m);
