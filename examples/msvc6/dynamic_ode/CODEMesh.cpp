													 ...
													 float a_y, float a_z, float a_density) 
{
   ...
   if ((l[0]>l[1]) && (l[0]>l[2]))
	...
	switch (a_type) {
		...
		case TRIMESH: {
						//Create the geometry associate to the body end then put it in the geometry space.
		...
		case BOX	: {
					   //Create a Box that wrap the mesh.
					   m_odeGeom = dCreateBox(m_odeSpace,l[0] ,l[1], l[2]);
					   break;
					  }

		case SPHERE	: {
					   //Create a Sphere that wrap the mesh.
...
//===========================================================================
/*!
    Create a cooy of the mesh compatible with ODE

    \fn     cODEMesh::cODEMeshToODE(unsigned int n,unsigned int &vertexcount,unsigned int &indexcount)
*/
//===========================================================================
